...
public class moserial.PreferencesDialog : GLib.Object
{
		...
		recordAutoName.set_tooltip_text (_("Filename is generated by time like moserial-yyyy-mm-dd_hh-MM-ss.txt and will be stored in the users home directory. When enabled, moserial won't ask for a filename when a recording is started."));

		recordAutoDirection = (ComboBox)builder.get_object("preferences_record_auto_direction");
		recordAutoDirection.set_tooltip_text(_("Select which direction should be recorded"));
		...
		pRecordAutoDirection = recordAutoDirection.get_active();
        	...
        	Preferences preferences=new Preferences(pSystemFont, pFont, pFontColor,pBackgroundColor,pHighlightColor, pRecordLaunch, pEnableTimeout, pTimeout, pRecordAutoName, pRecordAutoDirection);
